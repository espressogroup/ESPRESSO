...
import com.ibm.solid.Configuration;
...
//		"\n-w:	allows you specify the folder that will hold: gaiandb.log,  files and the node's database" +
	...
	public static final String getWorkspaceDir() {
		return GDB_WORKSPACE;
	}

	private static boolean isLiteNode = false;

	public static boolean isLite() {
		return false == IS_UDP_DRIVER_EXCLUDED_FROM_RELEASE && isLiteNode;
	}
...
//			runStatus = isShutdownComplete() ? RUN_STATUS_OFF : RUN_STATUS_PENDING_OFF;
	...
	public static boolean isRunning() {
		return RUN_STATUS_ON == getRunStatus();
	} // *ONLY* when RUN_STATUS_ON - otherwise watchdog won't exit
...
//		return isLite() || ( !IS_UDP_DRIVER_EXCLUDED_FROM_RELEASE && GaianDBConfig.getNetworkDriver().equals( GaianDBConfig.GDBUDP ) );
	...
	public static int getPID() {
		return null != gdbPID && gdbPID.matches("[\\d]+") ? Integer.parseInt(gdbPID) : 0;
	}

	private static SecurityClientAgent securityClientAgent = new SecurityClientAgent();

	public static SecurityClientAgent getSecurityClientAgent() {
		return securityClientAgent;
	}
...
//	}
	...
	public static void main(String[] args) throws Exception {
		...
		try {
			new GaianNode().start(args);
		} catch (Throwable e) {
			exitReason = e.toString();
		}
...
//		catch ( Throwable e ) { exitReason = e.toString(); }
	...
	public void start(final String[] args) throws Exception {
		...
		if (RUN_STATUS_OFF != getRunStatus())
			throw new Exception("Node must be in RUN_STATUS_OFF before it can be started, current status: " + runStatus);
...
//			}
			...
			try {
				...
				synchronized (gResults) {
					for (Iterator<GaianResult> gaianResultsIterator = gResults.iterator(); gaianResultsIterator.hasNext(); )
						try {
							gaianResultsIterator.next().close();
						} catch (Throwable e) {
							logger.logAlways("Unable to close a GaianResult, cause: " + Util.getStackTraceDigest(e));
						}
				}
			} catch (Throwable e) {
			...
			try {
				GaianResult.purgeThreadPools();
			} catch (Throwable e) {
				logger.logAlways("Unable to purge GaianResult Thread Pools, cause: " + Util.getStackTraceDigest(e));
			}
					...
					if (null != nsc) {
						...
						while (true) {
							try {
								nsc.ping();
								if (isFirstCheck)
									logger.logAlways("Waiting for DerbyNetworkServer to stop responding...");
								Thread.sleep(100);
								isFirstCheck = false;
							} catch (Exception e) {
								break;
							} // Server is gone... could also check that the actual tcp socket/port was released?
						}
					}
				} catch (Throwable e) {
					logger.logAlways("Unable to shutdown Derby Network Server, cause: " + Util.getStackTraceDigest(e));
				}
			}
...
//		GaianDBConfig.setWorkspace( mWorkspace );
		...
		try {
			showStartupTime(0, "PID resol0");
			processName = ManagementFactory.getRuntimeMXBean().getName();
			showStartupTime(0, "PID resol");
		} catch (Throwable e) {
			logger.logWarning(GDBMessages.NODE_PID_RESOLVE_ERROR, "Unable to resolve process name or pid (ignored): " + e);
		}
...
//				logger.logRaw("Network Server Ping...");
				try {
					...
					if (-1 < e.getMessage().indexOf("Keystore"))
						throw e; // Abort startup if we have incorrect SSL keystore or pwd
...
//					GaianDBConfig.getGaianNodeUser(), GaianDBConfig.getGaianNodePassword() );
			...
			try {
				...
				logger.logWarning(GDBMessages.NODE_LT_VIEW_UPDATE_ERROR, "Failed to update some logical table views (ignored): " + e);
			}
...
//		metricMonitor.addJVMMonitors();
		...
		try {
			getClass().getClassLoader().loadClass(MQTTMessageStorer.class.getName());
			isMessageStorerAvailable = true;
		} catch (Throwable e) {
			logger.logInfo("Could not load MessageStorer (install wmqtt.jar from IBM Microbroker): " + e);
		}
	}

	private static final PrintStream BIT_BUCKET = new PrintStream(new OutputStream() {
		@Override
...
//				catch ( Exception e ) { logger.logException("ENGINE_WATCHDOG_AUTO_LOAD_JDBC_DRIVER", "Unable to auto-load JDBC drivers", e); }
				...
				try {
					isDataServiceCidUpdatesWereRegistered = autoLoadGaianConnectionsForNewlyAvailableBlueMixRDBMSServices();
				} catch (Exception e) {
					logger.logException("ENGINE_WATCHDOG_AUTO_LOAD_RDBMS_CID_ERROR", "Unable to auto-load BlueMix RDBMS services", e);
				}
...
//					loadNewEntityAssociations();
				...
				for (int i = 0; i < THROUGHPUT_PERIODS_PER_WATCHDOG_PERIOD; i++) {
					...
					if (0 < numCancelled)
						logger.logThreadInfo("Number of queries cancelled by timeouts: " + numCancelled);
				...
				synchronized (oldvtis) {
					for (VTIWrapper dsWrapper : oldvtis)
						try {
							...
							logger.logException(GDBMessages.NODE_START_CLOSE_OLD_VTI_ERROR_SQL, "Unable to close old VTIWrapper: ", e);
						}
...
//				gdbNodeThreadGroup.list();
				...
				if (!isLite()) {
					if (Logger.LOG_NONE < Logger.logLevel) {
						if (false == logConnections) {
							nsc.logConnections(true);
							logConnections = true;
						}
					} else if (true == logConnections) {
...
//		metricMonitor.stop();
	...
	private class DynamicClassLoader extends URLClassLoader {
		public DynamicClassLoader(URLClassLoader cl) {
			super(cl.getURLs());
		}
...
//			final URLClassLoader cl = new URLClassLoader((URL[]) fileURLs.toArray(new URL[0]));
			...
			System.setOut(BIT_BUCKET);
			System.setErr(BIT_BUCKET);
			...
			for (URL url : jarURLs)
				for (Enumeration list = new ZipFile(url.getFile()).entries(); list.hasMoreElements(); numFiles++) {
					...
					try {
						loadedClasses.add(Class.forName(className, false, getGdbCL()));
					} // just load the class for now
					catch (Throwable e) {
						logger.logDetail("Unable to load class: " + className + ", cause: " + e);
					} // ignore/skip
				}
			...
			for (Class<?> candidateDriverClass : loadedClasses) {
				if (JDBC_DRIVER_INTERFACE.isAssignableFrom(candidateDriverClass)) {
					...
					try { // Explicitly register this driver - (DriverManager only pre-registers known drivers from system class-loader..)
						...
						logger.logDetail("Unable to register jdbc driver: " + candidateDriverClass.getName() + ", cause: " + e);
					} // ignore/skip
				}
			}
	...
	static Class<?> getClassUsingGaianClassLoader(final String className) throws Exception {
		...
		if (isLite()) return Class.forName(className);
			...
			try {
				...
				return Class.forName(className, true, gdbNodeSingleton.getGdbCL());
			}
		}
	}
			...
			for (String s : props) { // get each property
				...
				if (null != url && null != usr && null != pwd && url.startsWith("jdbc:")) {
					...
					if (null == availableConnectionsToSynchTo)
						availableConnectionsToSynchTo = new HashMap<String, String>();
...
//	public static int killNode( int exitCode ) { stopNode(exitCode); return 1; } // isKillRequested = true; killExitCode = exitCode; return 1; }
	...
	public static int stop(int exitCode) {
		stop("Explicit stop. Exit code: " + exitCode);
		GaianNode.exitCode = exitCode;
		return 1;
	}
...
//	public static int getKillExitCode() { return killExitCode; }
	...
	public static long getDataThroughput() {
		return dataThroughputInLastPeriod;
	}

	private static int queryActivityInLastPeriod = 0;

	public static int getQueryActivity() {
		return queryActivityInLastPeriod;
	}

	private static int nodeCPUInLastPeriod = 0;

	public static int getNodeCPUInLastPeriod() {
		return nodeCPUInLastPeriod;
	}
...
//	public static int getDsCpuInLastPeriod() { return dsCpuInLastPeriod; }
	...
	static {
		try {
			...
			logger.logWarning(GDBMessages.NODE_THREADMXBEAM_ERROR, "Unable to get threadMXBean - will not be able to compute CPU utilisation (ignored): " + e);
		}
	}
...
//						GaianDBConfig.getGaianNodeUser(), GaianDBConfig.getGaianNodePassword() );
	...
	private void setArgs(String[] args) {
		...
		for (int i = 0; i < args.length; i += 2) {
			...
			if ("-showtimes".equals(arg)) {
				showStartupTimes = true;
				i--;
				continue;
			}
...
//				mWorkspace = val;
				...
				if (null != mConfig || !val.endsWith(".properties"))
					syntaxError(args, "Config file name must be specified once only and have the extention '.properties'");
...
//				initsql = val;
				...
				if (null != mLogLevel /*|| null != mLogFile*/)
					syntaxError(args, "-log argument cannot be specified more than once");
