{
  "@context": [
    "https://linkedsoftwaredependencies.org/bundles/npm/@comunica/actor-rdf-resolve-quad-pattern-hypermedia/^2.0.0/components/context.jsonld"
  ],
  "@id": "npmd:@comunica/actor-rdf-resolve-quad-pattern-hypermedia",
  "@type": "Module",
  "requireName": "@comunica/actor-rdf-resolve-quad-pattern-hypermedia",
  "import": [
    "carrqph:components/ActorRdfResolveQuadPatternHypermedia.jsonld",
    "carrqph:components/MediatedQuadSource.jsonld"
  ]
}