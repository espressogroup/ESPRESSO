...
async function getWebIdData(baseUrl, webIdQuery, serverIndexUrl) {
    ...
    if (userWebIdData) {
        userHandle = Object.keys(userWebIdData)[0];
        userPods = userWebIdData[userHandle];
    }

    return { userPods, userHandle };
}
...
async function readSourcesWithSrvrMetadata(webIdQuery, keyword, baseUrl,metaIndexName,serverMetadata,indexType) {
    ...
    // Get keyword index data, if missing return empty list
    const keywordIndexData = await getKeywordFileData(serverIndexUrl, keyword,indexType);
    if (!keywordIndexData) return [];


    // Now, we Get WebID data i.e., pod_handles from user.webid
    const { userPods, userHandle } = await getWebIdData(baseUrl, webIdQuery, serverIndexUrl);
    if (!userHandle ) return [];


    let selectedPods = new Set();

    // Fetch the podlookup.json
    const podUrlMapperUrl = `${serverIndexUrl}podlookup.json`;
    const podUrlMapperResponse = await axios.get(podUrlMapperUrl).catch(err => null);
    const podUrls = podUrlMapperResponse ? podUrlMapperResponse.data : {};

    // Matched entries from .ndx file
    const userEntries = keywordIndexData[userHandle] || {};

    // Collect results (pod URLs with keyword appearances)
    for (const [podId, count] of Object.entries(userEntries)) {
        if (userPods.includes(podId)) {
            if (podUrls[podId]) {
                selectedPods.add(`${baseUrl}${podUrls[podId]}espressoindex/`);
            }
        }
    }
