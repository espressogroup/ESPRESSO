...
class PodIndex:
    ...
    Inverted Index class.
    """
    def __init__(self,namespace,podaddress,reprformat):
        self.index = Graph()
        self.n=0
        self.f=0
    ...
    def index_pod_file_text(self, id, text):
        """
        Process a given document, save it to the DB and update the index.
        """
        ...
        self.f=self.f+1
        filenode=BNode(fileword)
        self.index.add((filenode,self.namespace.address,URIRef(id)))
        for (key, appearance) in appearances_dict.items():
            if (None, None, Literal(key)) not in self.index:
                wordnode=BNode(key)
                self.index.add((wordnode,self.namespace.lemma,Literal(key)))
            for s, p, o in self.index.triples((None, self.namespace.lemma, Literal(key))):
                ...
                appnode=BNode(appword)
                self.index.add((s,self.namespace.appearsIn,appnode))
                self.index.add((appnode,self.namespace.file,filenode))
                self.index.add((appnode,self.namespace.frequency,Literal(appearance.frequency)))
...
def podlistindexer (filedict,namespace,podaddress,reprformat):
    index = PodIndex(namespace,podaddress,reprformat)
    for (filename, filetext) in filedict.items():
        print('indexing '+filename)
        index.index_pod_file_text(filename,filetext)
    return(index)